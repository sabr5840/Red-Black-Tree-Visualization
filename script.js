...
// This function searches for a node with the specified value in the Red-Black Tree and highlights it if found.
function searchNode() {
    const value = document.getElementById('nodeValue').value; // Retrieve the value from the input field with the id 'nodeValue'.
   
    // Check if the value is not provided or is not a valid number. 
    if (!value || isNaN(value)) {
        // Alert the user to enter a valid number.
        alert('Please enter a valid number.');
        return;
    ...
    // Search for the node with the given value in the Red-Black Tree.
    const node = tree.search(Number(value));
    renderTree(node); // Render the tree to visualize its current state.

    // If the node is found, highlight it; otherwise, alert that the node was not found.
    if (node !== null) {
        blinkNode(node); // Function to visually highlight the node.
    } else {
        alert('Node not found.'); // Inform the user if the node does not exist.
    }
}
